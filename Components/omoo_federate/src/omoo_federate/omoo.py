...
def Proj_inverter(xt, yt, Ux, Sx):
    ...
    """
    if Ux - Sx > 0:  # If available power is larger than capacity, use capacity
        Ux = Sx
...
def cost_fun(
    ...
    dLq_pp = q_cost + q_set_dev + (H[:, pv_ii]) @ (muk_last - lambdak_last)
    ...
    dLp_pp = p_cost + p_set_dev + (G[:, pv_ii]) @ (muk_last - lambdak_last)
...
def pv_cost(
    ...
    for pp, pv_ii in enumerate(pv_index):
        ...
        pv_avail_v = pv_frame.iloc[pp]["avai"] / Sbase
        P_set_v = (pv_frame.iloc[pp]["avai"] - pv_set_point_real[pp]) / Sbase
...
class OMOO:
    def __init__(
        ...
        if self.parameters.units == UnitSystem.PER_UNIT:
            base_power = 100
            if self.parameters.base_power is not None:
    ...
    def opf_run(self, V, P_wopv, Q_wopv):
        if self.parameters.units == UnitSystem.SI:
            ...
            raise Exception(f"Unit system {self.parameters.units} not supported")
        P_wopv, Q_wopv = (
...
class OMOOFederate:
    ...
    def run(self):
        ...
        if not isinstance(topology.admittance, AdmittanceMatrix) and not isinstance(
            ...
            raise ValueError("OMOO algorithm expects AdmittanceMatrix/Sparse as input")
        ...
        if not slack_index == 0:
            raise ValueError("Slack index is not 0")
        ...
        if self.algorithm_parameters.units == UnitSystem.PER_UNIT:
            base_power = 100
            if self.algorithm_parameters.base_power is not None:
        ...
        while granted_time < h.HELICS_TIME_MAXTIME:
            ...
            while granted_time <= np.floor(previous_time) + 1:
                granted_time = h.helicsFederateRequestTime(self.vfed, 1000)
...
def run_simulator(broker_config=None):
    """Entry point for running the OMOO federate."""
